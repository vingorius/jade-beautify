...
module.exports = {
	...
	convert: function() {
		var debug = false;
		if(debug) console.log(atom.config.get('jade-beautify.FILL_TAB'),atom.config.get('jade-beautify.OMIT_DIV'));
		...
		// it works only jade files.
		if(!editor.getPath().endsWith('.jade')){
			if(debug) console.log('not supporting.');
			return;
		}
